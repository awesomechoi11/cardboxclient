...
function SearchInner({
  ...
  const { isIdle, isError, isLoading, isSuccess, data, isFetching } = useQuery(
      ...
      <div tabIndex="-1" onFocus={onFocus} onBlur={onBlur}>
        <div className="relative rounded-md">
          ...
          <input
            ...
            className={clsx(
              errors.email && touched.email && errors.email && "border-red-500",
          ...
          {focused && value && (
            ...
            >
              <div className="relative top-1 border-[1px] bg-white border-gray-200 rounded-xl text-base p-1 flex flex-col gap-1 shadow-xl">
                {isFetching && !data && (
                  <div className="py-3 font-bold text-center">Loading</div>
                )}
                {!!data?.length && (
                  <>
                    {data?.map((result) => (
