...
export function CreatePackControlsTop() {
  ...
  return (
    <div className="create-pack-controls py-[16px] px-0 flex items-center mb-[64px] h-[40px] top-0 sticky z-10 ">
      <div className="left absolute w-full pointer-events-none flex items-center gap-[16px] ">
        <Button size="sm" onClick={() => setCards("swapAllCardTermDefinition")}>
          swap
        </Button>
      ...
      <div className="middle  absolute w-full pointer-events-none flex items-center gap-[16px] ">
        <span className="text-blue-600 font-bold mx-2 my-0">
          {cardIds?.length || 0} Total Cards
        </span>
      </div>
      <div className="right  absolute w-full pointer-events-none flex items-center gap-[16px] ">
        {saveState.saving ? (
          <div className="text-blue-500 font-bold my-2 mx-0">Saving...</div>
        ) : (
